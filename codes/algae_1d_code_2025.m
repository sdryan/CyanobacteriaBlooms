...
eta2 = 0.0; %Consumption Rate of P by Mike
zeta2 = .00228; %Consumption Rate of P by Ana
% Removing Competition Term for Now
beta1 = 0.0; %0.0001;%.0001; %competition coefficient Mike
beta2 = 0.0; %0.0001; %.0001; %competition coefficient Ana
CFL = alpha1*dt/(dx*dx)
f0 = 300.0;
...
for k = 1:Time/dt
    ...
    % Evolution
    for i = 2:M
        u(i,2) = u(i,1) + dt*(alpha1_nd*(u(i+1,1)-2.0*u(i,1)+u(i-1,1))/(dx*dx)+r1(i)*u(i,1)*(1.0-u(i,1))-delta_nd*u(i,1));
        w(i,2) = w(i,1) + dt*(alpha1_nd*(w(i+1,1)-2.0*w(i,1)+w(i-1,1))/(dx*dx)+r2(i)*w(i,1)*(1.0-w(i,1))-delta_nd*w(i,1));
        N(i,2) = N(i,1) + dt*(alpha2_nd*(N(i+1,1)-2.0*N(i,1)+N(i-1,1))/(dx*dx)-gamma_nd*N(i,1)-eta1_nd*N(i,1)*u(i,1)-zeta1_nd*N(i,1)*w(i,1)+f1(i,1)); 
        P(i,2) = P(i,1) + dt*(alpha2_nd*(P(i+1,1)-2.0*P(i,1)+P(i-1,1))/(dx*dx)-gamma_nd*P(i,1)-zeta2_nd*P(i,1)*w(i,1)+f2(i,1));
...
%         if (time_t1 > 0)
%         Tnew1 = [f0,time_t1*(t0)/24];
%         else
%         Tnew1 = [f0,Time*(t0)/24];
%         end
%         dlmwrite('nm_time_overtake_smallsource.dat', Tnew1,'delimiter',' ','-append');
%         if (time_t2 > 0)
%         Tnew2 = [f0,time_t2*(t0)/24];
%         else
%         Tnew2 = [f0,Time*(t0)/24];
%         end
%         dlmwrite('nm_time_form_smallsource.dat', Tnew2,'delimiter',' ', '-append');
%         if (time_t3 > 0)
%         Tnew3 = [f0,time_t3*(t0)/24];
%         else
%         Tnew3 = [f0,Time*(t0)/24];
%         end
%         dlmwrite('nm_time_decay_smallsource.dat', Tnew3,'delimiter',' ', '-append');
% end
