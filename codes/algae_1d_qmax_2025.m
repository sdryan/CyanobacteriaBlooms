...
% iterations
N_p = 61;  % How many variations of beta
for pp = 1:N_p
    count_t1 = 0;
    count_t2 = 0;
    count_t3 = 0;
...
qmax = (pp-1)*.001;%.007; %max absoprtion rate of chemical by Ana
...
kn = .53; %mug/L half way to critical threshold (Michaelis-Menten)
qn = .53; %mug/L half way to critical threshold (Michaelis-Menten)
...
beta1 = (pp-1)*.0005; %0.0001;%.0001; %competition coefficient Mike
beta2 = (pp-1)*.0005; %0.0001; %.0001; %competition coefficient Ana
CFL = alpha1*dt/(dx*dx)
   ...
   %x1 = 0:dx:L;
    ...
    elseif (i <= 3*M/8 && i > M/4)
       % f(i,1) = 1;  % Step function
        f(i,1) = 20.0*exp(-1.0/(1.0-(-1.0+(2.0)/(b1-a1)*(x1(i)-a1))^2));  %Nondim function scaling is t0/kn
...
% Write trial info to file
        if (time_t1 > 0)
        Tnew1 = [qmax,time_t1*(t0)/24];
        else
        Tnew1 = [qmax,Time*(t0)/24];
        end
        dlmwrite('qmax_time_overtake.txt', Tnew1,'delimiter',' ','-append');
        if (time_t2 > 0)
        Tnew2 = [qmax,time_t2*(t0)/24];
        else
        Tnew2 = [qmax,Time*(t0)/24];
        end
        dlmwrite('qmax_time_form.txt', Tnew2,'delimiter',' ', '-append');
        if (time_t3 > 0)
        Tnew3 = [qmax,time_t3*(t0)/24];
        else
        Tnew3 = [qmax,Time*(t0)/24];
        end
        dlmwrite('qmax_time_decay.txt', Tnew3,'delimiter',' ', '-append');
end
