...
% iterations
N_p = 31;  % How many variations of beta
for pp = 1:N_p
    count_t1 = 0;
    count_t2 = 0;
    count_t3 = 0;
...
% Write trial info to file
        if (time_t1 > 0)
        Tnew1 = [(pp-1)*.0005,time_t1*(t0)/24];
        else
        Tnew1 = [(pp-1)*.0005,Time*(t0)/24];
        end
        dlmwrite('comp_time_overtake.txt', Tnew1,'delimiter',' ','-append');
        if (time_t2 > 0)
        Tnew2 = [(pp-1)*.0005,time_t2*(t0)/24];
        else
        Tnew2 = [(pp-1)*.0005,Time*(t0)/24];
        end
        dlmwrite('comp_time_form.txt', Tnew2,'delimiter',' ', '-append');
        if (time_t3 > 0)
        Tnew3 = [(pp-1)*.0005,time_t3*(t0)/24];
        else
        Tnew3 = [(pp-1)*.0005,Time*(t0)/24];
        end
        dlmwrite('comp_time_decay.txt', Tnew3,'delimiter',' ', '-append');
end
